...
test('大会のラベルと出題形式が参加者の作成・編集・表示まで引き継がれる', async ({ page, request }) => {
  ...
  await page.getByRole('radio', { name: '選択肢2を正解にする', exact: true }).check();
  ...
  expect(saved.correctChoiceIndex).toBe(1);
  ...
  await expect(page.getByRole('radio', { name: '選択肢2を正解にする', exact: true })).toBeChecked();
  await page.getByRole('radio', { name: '選択肢4を正解にする', exact: true }).check();
  await page.getByLabel('選択肢数').fill('3');
  ...
  await expect(page.getByRole('region', { name: '正解の選択肢' })).toContainText('バイオリン');
  await expect(page.getByText('2. バイオリン', { exact: true })).toBeVisible();
  await evidence(page, '08-correct-answer');
});
