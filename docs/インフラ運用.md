...
2026-09-08時点。CloudFormationと実リソースを正とする。検証結果は[デプロイ検証記録](デプロイ検証記録.md)を参照。
...
`scripts/config.ts`の既定値は`gather-quiz-dev`、`ap-northeast-1`、`dev.gather-quiz.june-yamamoto.com`。
別環境では`GATHER_STACK`、`AWS_REGION`、`GATHER_DOMAIN`、`GATHER_ZONE_ID`を設定する。
...
既存アカウントでGitHub OIDCプロバイダーを共用する場合は、CIテンプレートを共用ARN参照に変更してから構築する。
...
3. ブランチは`main`、対象は`backend`、`frontend`、`both`から選ぶ。

mainへのpushだけではAWSを更新しない。OIDCはmainからの実行だけを許可する。
GitHubリポジトリ変数`AWS_DEPLOY_ROLE_ARN`にCIスタックの`DeploymentRoleArn`を設定する（今回設定済み）。
