...
describe('問題枠の設定', () => {
  it('同じ配点のラベルを問題順ごとに送信する', () => {
    ...
    expect(screen.queryByLabelText('1問目の選択肢数')).not.toBeInTheDocument();
    fireEvent.submit(screen.getByRole('button', { name: 'この内容で大会を作成する' }).closest('form')!);
    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({
      points: '10,10,30',
      questionSlots: [
        { label: '声優', choiceCount: 0, questionType: 'choice' },
        { label: '音楽', choiceCount: 0, questionType: 'normal' },
        { label: '', choiceCount: 0, questionType: 'normal' },
      ],
    }));
  });
});
