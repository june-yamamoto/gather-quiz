...
export async function buildBackend() {
  ...
  await writeFile(join(output, 'schema.correct-choice.sql'), (await readFile(join(backend, 'prisma/migrations/0004_correct_choice/schema.sql'), 'utf8')).replaceAll('\r\n', '\n'));
  await writeFile(join(output, 'migration.team-scoring.sql'), (await readFile(join(backend, 'prisma/migrations/0005_team_scoring/migration.sql'), 'utf8')).replaceAll('\r\n', '\n'));
